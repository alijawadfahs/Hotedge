...
\item[Robustness.] Kubernetes relies on control-theory principles
  ...
  types of failures and misbehaviours that are due to be even more
...
testbed \noteaf{testbed or test-bed?}composed of three Raspberry Pi~3 nodes running Kubernetes v1.9. As illustrated in Figure~\ref{fig:clus}, one
...
each other using a 1\,Gbps Ethernet local-area network\footnote{The
  local-area network infrastructure guarantees a 1\,Gbps capacity
...
When creating a new pod the scheduler should know about the end-user
locations (approximated by the locations of gateways they are
connected to). Then we can use latency-driven replica
placement~\cite{szymaniak2006}. It's a dynamic system so it may be
useful to adapt the auto-scaler (or to design an entirely new
controller) that periodically estimates how efficient the current
replica placement is, and initiates adjustments if necessary.

Longer-term goal: decentralizing the scheduler itself.
