...
The master can schedule each pod in any worker node that has
sufficient available resources. Since pods may be placed in different
nodes, communication is established as follows:

\begin{itemize}
\item Pod-to-pod communication is enabled using Software Defined
  Networking (SDN) \notegp{I presume this means creating a separate
    VPN for the pods which will be implemented using
    IPtables. Correct?}
\item Communication between the end users and the pods is ensured by
  by the Kubernetes service together with a component called
  \emph{kube-proxy} which also relies on iptables to establish
  communication.
\end{itemize}
...
redirected to is determined randomly by default. Alternatively one may
decide to use a third-party load balancer. \notegp{Do we have examples
  of such 3rd-party load balancers? Is there any location-aware one in
  the list?}
