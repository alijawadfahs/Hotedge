...
K8's cluster is composed of master node and worker nodes, all the decision of the cluster are made by the master, and all the cluster controllers are located there. This impose this imposes higher latencies when executing jobs like deploying a container, also heavy computation are done in the master node to take all the decision of the cluster.\change{rephrase!}  
...
Load balancing in kubernetes is limited to external load balancers used in the cloud providers, since the fog is definitely not built in the cloud provider hardware then now load balancing options are available for the fog.\unsure{need to check more}

\item {\bf Scheduling regardless the location}
The deployment of the containers is not related to the location of the node neither, the scheduling of the incoming requests from the application users. As a result we will end up by the jobs traversing more hops in the network before reaching the targeted nodes, which implies higher end-to-end latencies.\change{rephrase!}
